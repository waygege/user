...
export default {
  ...
  computed: {
    ...
    allpay () {
      ...
      for (let i = 0; i < selectedList.length; i++) {
        allpay += selectedList[i].salePrice
      }
    allpay=allpay-this.couponsSize;
      return allpay
    }
  },
  mounted () {
    this.couponsSize=sessionStorage.getItem("size")
    // 防止页面刷新后数据丢失
  ...
  methods: {
    payConfirm () {
      ...
      this.couponsId=sessionStorage.getItem("couponsId")
      if (this.carList) { //还未提交了订单,数据还未清空
        MessageBox
          ...
          .then(action => {//点击成功执行这里的函数
            ...
            this.$api({
              method: 'get',
              url: '/alipay/pay?payMoney='+this.allpay+'&userId='+this.userId+'&couponsId='+ this.couponsId
            }).then((response) => {
