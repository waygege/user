<template lang="html">
  <footer class="footer">
    ...
    <router-link  class="footer-goon" :to="{ name: '使用优惠卷'}">
     优惠卷
    </router-link>
...
export default {
    ...
    allpay () {
      ...
      if (this.$store.getters.selectedList != undefined) {
        ...
        for (let i = 0; i < this.$store.getters.selectedList.length; i++) {
          ...
          all += this.$store.getters.selectedList[i].salePrice;
  ...
  methods: {
    ...
    goPay () {
      ...
      this.transationAmount.payMoney=this.allpay
      // 如果有选择商品才能跳转
      if (this.$store.getters.selectedList.length) {
        ...
        /*this.$api({
          method: 'get',
          url: '/alipay/pay?payMoney='+this.allpay
        }).then((response) => {*/
